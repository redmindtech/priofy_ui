 ...
 this.currentUser = storedUser ? JSON.parse(storedUser) : null;
  ...
  formInitialization() {
    this.ChecklistF = this.formBuilder.group({
      iot_move_furnace_sequence_to_Swing_text:['1.IOT to move furnace sequence to Swing MOV step, if not done before.'],
      iot_confirm_text:['2.IOT to confirm that HV-22X0-02 (purge steam to ethane feed line(FV-22X0-14 A/B are open) opens to 100% automatically.)'],
      oot_ALL_BV_of_LS_steam_text:['3.OOT to ensure that ALL B/V of LS steam to PTs and MOVs are lined up.<br>Note: automated valve to CG MOV intermediate PT will not be auto enabled until 2 seconds after feed EBV is opened and will stay open for 120 seconds after the feed EBV closes.'],
      oot_flare_block_text:['4.OOT to confirm that both B/V of the CG MOVs purge steam lines drain valves and the flare block and bleed valves are closed.'],

      oot_IOT_Decoke_Air_text: ['5.OOT&IOT to reconfirm that Decoke Air is blinded.'],
      oot_feed_DB_B_text: ['6.OOT to confirm that ethane feed DB&B are closed.'],
      iot_MOVs_status_text: ['7.IOT to confirm that ALL MOVs status signal are ok.'],
      oot_MOVs_local_switches_text: ['8.OOT to check and confirm that all the MOV’s local switches are in “auto” position.'],
      oot_furnace_is_clear_text: ['9.OOT to ensure the furnace is clear of all personnel before the MOV swing starts.'],
      iot_Operator_Permissives_text: ['10.IOT to initiate the MOV swing via HMI by selecting “Operator Permissives to Swing to CG header”.<br>Note: First make sure to manually RESET the BM & BNR alarms (unlatch).'],
      iot_second_CG_MOV_text: ['11.IOT to confirm that second CG MOV fully opens (HV-22X6-13B).'],
      iot_Decoke_MOV_text: ['12.IOT to confirm that Decoke MOV (HV-22X6-14) begins to close after second CG MOV has opened.<br>Note: it will close to approximately 50% without ramp or until the pressure reach the higher-pressure set point (set point for closing MOV).'],
      iot_higher_pressure_text: ['13.IOT to confirm that Decoke MOV continues closing by delayed pulses to achieve the higher-pressure set point (set point for closing MOV).'],
      iot_lower_pressure_text: ['14.IOT to confirm that primary CG MOV (HV-22X6-13A) opens by delayed pulses to achieve the lower pressure set point (set point for opening MOV).'],
      iot_decoke_MOV_closed_text: ['15.IOT to confirm that MOVs continue to open/close until the Decoke MOV is fully closed.'],
      iot_confirm_via_HMI_text: ['16.IOT to confirm via HMI that the Decoke MOV is fully closed after OOT confirms this in the field.<br>Note: this confirmation allows the IOT to open out primary CG MOV to fully open without delay pulses.'],
      iot_HSSB_Crack_Gas_step_text: ['17.IOT to move the furnace sequence to HSSB Crack Gas step after the CG MOV is fully open.'],
      adjust_combustion_text: ['18.If COT increase, follow the below step:Adjust combustion air flow.'],
      the_top_burners_text: ['18.If COT increase, follow the below step:A Cut some of the top burners if fuel gas pressure reaches 0.2 kg/cm2.'],
      increase_IBD_CBD_text: ['18.If COT increase, follow the below step:Increase IBD/ CBD.'],
      continue_with_Furnace_text: ['19.Continue with Furnace, Feed In procedure.'],
