...
formInitialization(){
  this.ChecklistC= this.fb.group({
    ...
    iot_furnace_control_text:['1.Has IOT  manually  moved the BM sequence to Stand By step and also manually  moved the Furnace control sequence moves to Purge & Light Off step. '],
       // oot_reset_IOTmove_BM_sequence:[' '],
    pressure_test_text:['2.If the pressure test and hold pressure (holding time = 30 seconds) is performed in this step prior to the pad/depad. Then ,does pad/depad to atmosphere is done 6 times in “auto”. If purge permissive are not maintained or pressure up & holding do not meet the requirements, furnace will trip.'],
    iot_fuel_header_purge_text:['3. IOTs ensure that whether BM sequence returns back to Standby step after Fuel Header Purge step is done.'],

    oot_adjacent_furnaces_area_text:['4.OOT check and  make sure that the furnace to be lit and the adjacent furnaces area is cleared of all personnel, including the Furnace OOT. '],

    iot_request_local_reset_text:['5.After one purge volume is completed, Has IOT  requested OOT to push local reset button (field flashing green light should come on solid). '],

    iot_move_pressure_test_text:['6.Has IOT moved BM sequence to Pressure Test step within 15 minutes of field reset. '],

    iot_pressure_up_text:['7.Has IOT  observed pressure test of fuel gas system.Step1: Pressure Up: bypass DB&B opens, local PCV opens and header is pressurized to 0.2 kg/cm2g within 100 seconds, or trip occurs.'],

    iot_Hold_text:[' Step2:Hold: bypass DB&B closes, pressure must not drop more than 0.02 kg/cm2 (10%) or trip occurs.'],

    oot_pressure_test_text:['8.If Pressure Test fails, has OOT  de-pressured fuel gas headers to radiant box. '],

    pressure_test_BM_sequence_text:['9.When Pressure Test is complete BM sequence moves automatically to Firebox Purge step. '],

    oot_4_automated_burners_text:['10.Has the OOT ensured that all floor burners air registers are fully open, except the 4 automated burners that must be closed. '],

    oot_wall_burners_text:['11.Has OOT  ensured that all wall burners air registers are closed. '],

    bm_sequence_moves_purge_text:['12.After 5 purge volumes are completed, does the BM sequence moves automatically to Purge Complete step. '],

    iot_steam_drum_level_text:['13.Has IOT  ensured that ALL parameters below are OK so he can initiate the light off.Step1:*Steam drum level.'],

    iot_firebox_draft_text:['Step2:*Firebox draft.'],

    iot_automated_burners_text:['Step3:*All automated burners are ready for light off. '],

    iot_purge_permissive_text:['Step4:*purge permissive true. '],

    iot_no_combustibles_text:['Step5:*No combustibles in firebox.'],

    iot_fuel_control_text:['Step6:*Fuel control valves closed.'],

    iot_total_trip_text:['14.Has IOT  moved BM sequence to Light off Burners step within 15 minutes of Purge complete otherwise the furnace will go to Total    Trip.'],

    iot_to_manually_RESET_text:['15.Has IOT  manually RESET the BM & BNR alarms (unlatch). The remote burner light off cycle is automatically initiated in this step. Auto burner Light off attempt sequence is #7, #30, #21 and #16. '],

    one_burner_bms_text:['16.Do Ensure that atleast one burner in each zone must be lit to move the Burner Management sequence to the Run step. '],

    oot_adjust_air_damper_text:['17.Has OOT  adjusted the air damper for the automated burners which are lit to one notch for steady flame. '],

    oot_igniters_not_stuck_text:['18.Has OOT  ensured Igniters are not stuck inside the firebox. '],

    iot_unsuccessful_light_text:['19.Step1:Has IOT  confirmed BNR sequence moves after light off attempt:Unsuccessful light, sequence moves to total Trip step (HV closes). '],

    iot_successful_light_text:['Step2:Successful light, sequence can be moved to Run step by IOT '],


    furnace_failed_text:['20.If the Light off of the furnace failed, has OOT  manually depressurized the burner headers. '],

    iot_manually_move_BM_sequence_text:['21.Has IOT  manually move BM sequence to Run step if successfully achieved at least one burner per zone '],

    iot_to_confirm_text:['22.Has IOT confirmed that Furnace sequence has auto moved to Warm Up Arch step. '],

    e_attempted_via_the_HMI_text:['23.Has Auto burners that did not successfully light during the auto-light cycle, have it  been re-attempted via the HMI '],
