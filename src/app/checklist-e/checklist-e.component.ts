...
export class ChecklistEComponent implements OnInit {
  ...
  formInitialization() {
    this.ChecklistE = this.formBuilder.group({
      ...
      oot_cbd_confirm_cooling_water_id:['5.When steam drum pressure is above 90 kg/cm2g:OOT to ensure steam drum blowdown analyzer online and confirm cooling water flow to and from cooler.'],
  ...
  patchvalue(){
    ...
    this.ChecklistE.patchValue({
      ...
      oot_cbd_confirm_cooling_water_comment:this.concatenateValues(this.ChecklistE.get('oot_cbd_confirm_cooling_water_id')?.value,this.ChecklistE.get('oot_cbd_confirm_cooling_water_comment')?.value ),
