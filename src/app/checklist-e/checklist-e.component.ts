...
export class ChecklistEComponent implements OnInit {
  ...
  formInitialization() {
    this.ChecklistE = this.formBuilder.group({
      oot_lower_level_burner_text:['1.OOT to ensure that all lower level burners are lit before starting light off of upper level burners'],
      iot_monitor_the_fuel_gas_text:['2.IOT to continuously monitor the fuel gas pressure and OOT to continue lighting off more burners as required'],
      oot_desuperheater_text:['3.OOT to open desuperheater Untreated BFW manual block valves (DB&B) when the COT is &gt; 450 ºC.'],
    
    
      iot_desuperheater_not_pass_text:['4.IOT to confirm that the desuperheater valve does not pass. If it passes, instruct the OOT to close the B/V until desuperheating flow is needed.'],
      oot_lineup_sd_text:['5.When steam drum pressure is above 90 kg/cm2g:OOT to line up steam drum continuous blowdown valve and block in steam drum intermittent blowdown.'],
      oot_cbd_text:['5.When steam drum pressure is above 90 kg/cm2g:OOT to confirm that the sTLE blowdown to CBD header is lined up.'],
      oot_confirm_cooling_water_text:['5.When steam drum pressure is above 90 kg/cm2g:OOT to ensure steam drum blowdown analyzer online and confirm cooling water flow to and from cooler.'],
      oot_stle_blowdown_analyzer_text:['5.When steam drum pressure is above 90 kg/cm2g:OOT to ensure sTLE blowdown analyzer cooling water flow to and from cooler is established.'],
      oot_desuperheater_untreated_bfw_text:['5.When steam drum pressure is above 90 kg/cm2g: OOT to open desuperheater untreated BFW manual block valves (DB&B) if not done before.'],
      furnace_sequence_to_swing_mov_text:['8.When COT temperature is above 760ºC and if steam flow confirmed higher than 10 Mt/h per stexte, it is possible to move the Furnace sequence to Swing MOV step, if ready to do so.'],
      ootIOT_decoke_air_text:['7.OOT&IOT to reconfirm that Decoke Air is blinded (ONIS device).'],
      psv_evt_complete_text:['6.Continue with warm-up when PSV EVT testing is complete'],
      psv_evt_start_text:['6.If steam PSV EVT testing needs to be done, hold COTs at 700°C (via COT manual set point) and steam drum pressure at 93kg/cm2g using steam vent to atmosphere control valve (flow should be around 12 M t/h).'],
      iot_to_enable_sd_text:['5.When steam drum pressure is above 90 kg/cm2g:IOT to enable steam drum and sTLE continuous blow downs from HMI Station.'],
