...
export class ChecklistDComponent implements OnInit {
  ...
  formInitialization(){
    ...
    this.ChecklistD = this.fb.group({
      ...
      iot_arch_temperature_text:['1.Has IOTs confirm that “arch temperature” warm up ramp set point is 0.83 ºC/min (equivalent to 50 ºC/hr or to increase 200 ºC/hr in 4 hours) and that all fuel controllers are properly enabled and in the correct mode/mode attribute. '],
      oot_air_ONIS_blind_text:['2.OOT to open decoke air ONIS blind and 6” main B/V'],
      oot_to_open4_text:['3.COOT to open 4” decoke air B/V downstream each control valve FV-22X0-10A/B. '],
      iot_start_decoke_text:['4.Note: IOT to inform Utility before start decoke airflow IOT to enable Decoke Air controllers and start flow through the coils at the default set point of 3.0 Mt/h per pass (side) by resetting HV-22X0-07 through its HS.'],
      oot_fuel_gas_text:['5.OOT to light manual burners as requested by IOT (to maintain fuel gas pressure between 0.2 to 0.4 kg/cm2g) and by following Furnace, Burner Light off procedure.step1:Note 1: fuel gas pressure needs to be maintained as low as possible to keep the flame close to the tile when steam enters the firebox, in order to allow the scanner to see the flame.'],
      oot_burner_light_off_text:['Step2:burner light off sequence attached in Appendix A must be followed. '],
      oot_add_burners_text:['Step3:“add burners” alarm set point is 0.5 kg/cm2g (stops the ramp) and “cut burners” alarm is 0.2 kg/cm2g. Low-pressure constraint controller set point is 0.15 kg/cm2g. '],
      air_registers_3Notches_text:['6.When 10 burners are lit the air registers of the lit burners should now be opened to 3 notches. '],
      burners_20_text:['7.When around 20 burners are lit all the lit burners air registers  should be opened out to 100%.'],
      iot_to_request_OOT_text:['8.IOT to request OOT to close the 3” steam drum start up vents when the steam drum pressure is 5 kg/cm2g. '],
      iot_COTs_rising_text:['9.Step1:*Checking that all COTs are rising at about the same rate.'],
      iot_venturi_Ratios_text:['Step2:*Checking that all Venturi Ratios are about the same value.'],
      iot_suspect_coils_text:['10.IOT to clear pluggage from suspect coils by increasing the air flow (max. 3.6 Mt/h) on that side of the furnace. '],
      oot_Start_warm_MS_steam_text:['11.Keep ¾” bleed cracked open until the line is warm (dry steam coming from the drain).'],
      oot_spool_piece_text:['12.OOT to drain spool piece between 8” B/V and HV-22X0-06 to pad and then line up steam trap to MC header.Keep ¾” bleed cracked open until the line is warm (dry steam coming from the drain). '],
      oot_open_MS_EBV_text:['13.OOT to open MS EBV, HV-22X0-06. '],
      oot_topen_MS_steam8_text:['14.Once condensate has been drained OOT to open MS steam 8” B/V. '],
      iot_both_steam_text:['15.IOT to make sure that both steam control valves FV-22X0-22A/B remain closed. '],
      oot_steam_lines_up_text:['16.OOT to warm up the steam lines up to steam control valves Step1:*Keep ¾” bleed upstream each FV and ¾” bleed upstream 10” spectable blind cracked open until the lines are warm (dry steam coming from the drain).FV-22X0-22A/B. '],
      oot_dilution_steam_text:['17.OOT to drain the Dilution Steam to FPH Outlet lines (Pass 1&2) by opening the ¾” bleed downstream each FV. Keep ¾” bleed cracked open until the lines are warm (dry steam coming from the drain). '],
      oot_open2_steam_text:['18.OOT to open 2” steam B/V to ethane feed line and drain condensate by opening the ¾” bleed to pad upstream of HV-22X0-02. Keep ¾” bleed cracked open until the lines are warm (dry steam coming from the drain).</td> '],
      oot_open_3_4_ethane_text:['19.OOT to open 3/4” bleed downstream ethane feed HV-22X0-86 to pad.Keep ¾” bleed cracked open until the line is warm (dry steam coming from the drain). '],
      oot_crack_open_10_b_v_text:['20.OOT to crack open 10” B/V downstream each ethane feed control valve FV-22X0-14A/B and drain the ethane line to FPH backwards by opening the ¾” bleed downstream the FV to pad. Keep ¾” bleeds cracked open until the line is warm (dry steam coming from the drain).<br>When all the condensate is drained out close the drains and the 10” block valves.'],
      open_the_upstream_and_downstream_text:['21.Step1:Open the upstream and downstream 4” B/V of FV-22X0-08 and start closing the 2” bypass then take the control using the control valve FV-22X0-08. '],
      Phosphate_and_Morpholine_text:['Step2:Phosphate and Morpholine injection to main BFW line to be started. Dosing rate to be decided based on blowdown sample.<br>Note: Separate injection point valves for old F-2280 line should be kept closed. '],
      before_starting_text:['22.Before starting the steam flow through the coils the combustion air flow to the furnace FC-22X2-15D-16D should be increased to 60 M t/h to dilute the effect of steam on the scanners. IOT to enter manual set point. '],
      iot_reduce_draf_text:['23.IOT to reduce the draft to (-8) mmH2O by entering a manual draft set point, before starting the steam flow through the coils. '],
      iot_visually_check_the_firebox_text:['24.Step1:*Request the OOT to visually check the firebox to see if it starts to look foggy due to steam. '],
      iot_htc_1_outside_temperature_text:['Step2:*Monitor the steam flow reading to each side (FT-22X0-22A/B), the HTC-1 outside temperature on each side (TT-22X0-17A/B) and the radiant inlet temperature (TT-22X0-19A/B) and pressure (PT-22X0-18A/B) on each side to identify if steam is flowing or not yet.'],

      oot_ds_and_steam_to_fph_text:['25.OOT to line up steam lines purges (DS and steam to FPH) to C-2317 once all the steam lines are confirmed dry. '],

      iot_checking_that_all_COTs_text:['26.Step1:*Checking that all COTs are rising at about the same rate. '],

      iot_checking_that_all_venture_ratios_text:['Step2:*Checking that all venture ratios are about the same value. '],

      iot_to_clear_pluggage_text:['27.IOT to clear pluggage from suspect coils by slowly increasing the steam flow on that side of the furnace.'],
      ootsecondary_tle_text:['28.Step1:*Secondary TLE inlet PT '],
      oottertiary_tle_text:['Step2:*Tertiary TLE outlet PTs.'],
      oot_pt_between_both_CG_movs_text:['Step3: *PT between both CG MOVs.'],

      oot_CG_Decoke_MOV_text:['Step4:*Both CG MOVs and Decoke MOV seats (HV-22X0-13A and HV-22X0-14 and Cavity). '],

      iot_shp_steamflow_i_text:['29.IOT to ensure that SHP steam flow is exiting the vent silencer. '],

      iot_monitor_fuelgas_text:['30.IOT to continuously monitor the fuel gas pressure and OOT to continue lighting off more burners as required. '],
      oot_monitor_the_firebox_text:['31.OOT to continuously monitor the firebox to see if it starts to look foggy due to steam entering it. '],

      thesteamflow_is_greaterthan_10_text:['32.When the steam flow is >10 T/H/coil the decoke air flow is backed off by closing the FV’S in auto. '],
      oot_close_decoke_air_text:['33.OOT to close decoke air ONIS blind and 6” main B/V once there is no more air flow to coils. '],
      oot_to_close_4decoke_text:['34.OOT to close 4” decoke air B/V downstream each control valve FV-22X0-10A/B.'],
      iot_disable_decokeair_controller_text:['35.IOT to disable Decoke Air controllers and confirm that the ONIS “Close DI” is true. '],
      hv_22x0_07_isclosed_text:['36.When Decoke air HV-22X0-07 is closed HV-22X0-02 (purge steam to ethane feed line) should open 25% on program logic. '],

      oot_to_fully_open_downstream_text:['37.OOT to fully open downstream block valves of the ethane feed control valves FV-22X0-14A/B and metallic car seal provided. '],
      iot_to_confirm_ethane_feed_text:['38.IOT to confirm that ethane feed control valves FV-22X0-14A/B come fully open in auto to allow starting steam flow through the feed preheat coils. '],
     oot_to_lineup_steamdrum_text:['39.OOT to line up steam drum and sTLE continuous blowdown to header. When steam drum header pressure reads > 12 kg/cm2. '],

      adjust_the_steam_drum_blowdown_text:['40.If needed, adjust the steam drum blow down to maintain the BFW economizer coil outlet temperature below 312ºC. '],
      furnace_sequence_automove_to_warmup_text:['41.IOT to ensure Furnace sequence auto-move to Warm up COT HSSB step occurs. Warm up continues at 50°C per hour. '],
